class Stage2Manager {
    ...
    updateFactInputHighlight(input) {
        ...
        const factQty = parseFloat(input.value);
        const originalQty = parseFloat(input.dataset.originalQty);

        const hasUserDifference = !Number.isNaN(factQty) && !Number.isNaN(originalQty)
            ? factQty !== originalQty
            : String(input.value || '').trim() !== '';

        const sentRaw = (input.dataset.sentQty ?? '').toString().trim();
        const deliveredRaw = (input.dataset.deliveredQty ?? '').toString().trim();
        ...
        if (row) {
            if (hasUserDifference || hasSentDeliveredDifference) {
                row.classList.add('row-warning');
                // also set inline background to ensure visibility in all themes
                row.style.backgroundColor = '#ff000078';
            } else {
                row.classList.remove('row-warning');
                // remove inline background if previously set
    ...
    backToEdit() {
        ...
        const highlighted = document.querySelectorAll('#tableBody tr.row-warning');
